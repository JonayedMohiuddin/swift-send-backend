...
--     TOTAL_RATING        NUMBER DEFAULT 0,
    ...
    CONSTRAINT PRODUCT_SUPPLIER_FK FOREIGN KEY(SUPPLIER_ID) REFERENCES SUPPLIER(ID) ON DELETE SET NULL,
...
CREATE TABLE ORDER_ITEM (
    ...
    CONSTRAINT ORDER_ITEM_PRODUCT_FK FOREIGN KEY(PRODUCT_ID) REFERENCES PRODUCT(ID) ON DELETE CASCADE
);
...
CREATE TABLE SUPPLIER_ORDERS (
    ...
    CONSTRAINT SUPPLIER_PENDING_ORDERS_SUPPLIER_FK FOREIGN KEY(SUPPLIER_ID) REFERENCES SUPPLIER(ID) ON DELETE CASCADE,
    CONSTRAINT SUPPLIER_PENDING_ORDERS_USER_FK FOREIGN KEY(USER_ID) REFERENCES USERS(ID) ON DELETE SET NULL,
...
BEGIN 
    ...
    INSERT INTO LOGS(MESSAGE) VALUES('ORDER CANCELED FOR ' || PRODUCT_NAME);
...
CREATE OR REPLACE PROCEDURE ADD_ORDERS_FROM_CART (USER_ID_PARAM IN NUMBER, ADDRESS_PARAM IN VARCHAR2 DEFAULT NULL, PHONE_PARAM IN VARCHAR2 DEFAULT NULL) IS
    CART_ID_VALUE  NUMBER;
    ORDER_ID_VALUE NUMBER;
    ADDRESS_VALUE  VARCHAR2(1000);
	PHONE_VALUE    VARCHAR2(200);
BEGIN 
	...
	IF PHONE_PARAM IS NULL THEN
        SELECT PHONE INTO PHONE_VALUE FROM USERS WHERE ID = USER_ID_PARAM;
    ELSE
        PHONE_VALUE := PHONE_PARAM;
    END IF;

    INSERT INTO ORDERS(USER_ID, ADDRESS, PHONE) VALUES(USER_ID_PARAM, ADDRESS_VALUE, PHONE_VALUE) RETURNING ID INTO ORDER_ID_VALUE;
    ...
    INSERT INTO LOGS(MESSAGE) VALUES('ORDER PLACED FROM CART FOR USER ID ' || USER_ID_PARAM);
END;
/
...
BEGIN
    ...
    INSERT INTO LOGS(MESSAGE) VALUES('SUPPLIER REMOVED WITH ID ' || SUPPLIER_ID_PARAM);
EXCEPTION
    WHEN NO_DATA_FOUND THEN
...
BEGIN
    ...
    INSERT INTO LOGS(MESSAGE) VALUES('SUPPLIER ORDER PLACED FOR PRODUCT ID ' || :NEW.PRODUCT_ID);
EXCEPTION
    WHEN NO_DATA_FOUND THEN
...
BEGIN
    ...
    IF :OLD.STATUS = 'PENDING' AND :NEW.STATUS = 'SHIPPED' THEN
        ...
        INSERT INTO SWIFT_SEND_WAREHOUSE_ITEM(ORDER_ITEM_ID, USER_ID, ADDRESS, PHONE) VALUES(:OLD.ORDER_ITEM_ID, :OLD.USER_ID, :OLD.ADDRESS, PHONE_VALUE);
    
    ELSIF :OLD.STATUS = 'PENDING' AND :NEW.STATUS = 'CANCELED' THEN
        UPDATE ORDER_ITEM SET STATUS = 'CANCELED' WHERE ID = :OLD.ORDER_ITEM_ID;
    END IF;

    INSERT INTO LOGS(MESSAGE) VALUES('SUPPLIER SHIPPED ORDER FOR PRODUCT ID ' || :OLD.ORDER_ITEM_ID);
END;
/
