...
EXCEPTION 
    WHEN NO_DATA_FOUND THEN
		MSG := 'NOT FOUND 404. INTERNAL SERVER ERROR';
		RETURN MSG;
	WHEN OTHERS THEN
...
BEGIN
    ...
    INSERT INTO SUPPLIER_PENDING_ORDERS(SUPPLIER_ID, USER_ID, ORDER_ITEM_ID, ADDRESS) VALUES(
        SUPPLIER_ID_VALUE,
        USER_ID_VALUE,
        :NEW.ID,
        ADDRESS_VALUE
...
BEGIN
	...
	IF UPPER(ORDER_STATUS) = 'CANCELED' THEN
		RETURN;
	END IF;
	
    INSERT INTO SWIFT_SEND_WAREHOUSE_ITEM (ORDER_ITEM_ID, USER_ID, ADDRESS) VALUES(
		:OLD.ORDER_ITEM_ID,
        :OLD.USER_ID,
		:OLD.ADDRESS
    );

    UPDATE ORDER_ITEM SET STATUS = 'SHIPPED' WHERE ID = :OLD.ORDER_ITEM_ID;
END;
/
